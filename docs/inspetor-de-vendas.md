...
`api/go.ts` já registra cada clique em `cliques_rastreavel` com `source`, `campaign`,
`ad` e redireciona para `wa.me/<telefone>?text=<mensagem>`. O que **falta** é ligar
esse clique à conversa que nasce dele — hoje `conversa_id` fica sempre `null` no
registro do clique.

### 3.2. O elo que falta: casar clique → conversa

O truque é o **texto pré-preenchido** do `wa.me`. Podemos embutir um código curto
de rastreio na mensagem (`t=`), por exemplo:

```
Olá! Vim pelo anúncio [#a1b2c3]
```
...
2. No `webhook-evolution.ts`, ao receber a **primeira mensagem** de uma conversa,
   ...
   - gravar `origem`, `campaign`, `ad` na conversa;
...
### 3.3. Rastreamento no site (pixel próprio) — opcional

Se o objetivo for capturar o clique **antes** do WhatsApp (numa landing page do
cliente), dá pra servir um JS leve a partir da própria Vercel que:

- lê `utm_source`, `utm_campaign`, `utm_content`, `gclid` (Google) e `fbclid` (Meta)
  da URL;
- monta o link `/go/<instancia>` já com esses parâmetros;
- assim o `gclid`/`fbclid` chega ao Bob CRM e pode ir junto no evento da Meta.

Isso é o equivalente ao `tintim-1.0.js`. Só vale a pena se houver landing pages
próprias; se todo o tráfego já vai direto pro `/go/...`, o passo 3.2 basta.

> **Nota sobre atribuição por anúncio (Meta):** para atribuir a conversão ao anúncio
> específico de clique-para-WhatsApp seria preciso capturar o `ctwa_clid` do
> *referral* da primeira mensagem (hoje não coletado) e usar
> `action_source: "business_messaging"`. Está fora do escopo desta fase — ver
> `CLAUDE.md`, decisão do `system_generated`.
